...
#     for i in range(len(plrs)):
#         if method == 'kernel':
#             val = kernel_CKA(glob_plr, plrs[i].detach().cpu())
#         if method == 'linear':
#             val = linear_CKA(glob_plr, plrs[i].detach().cpu())
#         if method == 'mmd':
#             val = mmd_rbf(glob_plr, plrs[i].detach().cpu())
#         if method == 'cosine':
...
#     kmeans = KMeans(n_clusters=2, random_state=0).fit(np.array(sim_vals).reshape(-1, 1))
#     labels = kmeans.labels_
#     counter = Counter(labels)
#     print(counter)
...
#     suspects = np.where(labels == suspect)[0]
#     scale = sim_vals
...
#     selected_idx = np.where(np.array(scale) > 0)[0]

#     print(f'len(selected_parameters) = {len(selected_parameters)}')
...
def fed_align(flat_weights, plrs):
    num_clients = len(plrs)
    
    align_loss_vals = np.zeros((num_clients, num_clients))
    # linear_cka = np.zeros((num_clients, num_clients))
    # kernel_cka = np.zeros((num_clients, num_clients))

    for i in range(len(plrs)):
        # print(f'uniform loss plrs[{i}]: {uniform_loss(plrs[i])}')
        for j in range(i+1, len(plrs)):

            align_loss_vals[i][j] = align_loss(plrs[i], plrs[j])
            align_loss_vals[j][i] = align_loss_vals[i][j]

#             linear_cka[i][j] = linear_CKA(plrs[i].detach().cpu(), plrs[j].detach().cpu())
#             linear_cka[j][i] = linear_cka[i][j]

#             kernel_cka[i][j] = kernel_CKA(plrs[i].detach().cpu(), plrs[j].detach().cpu())
#             kernel_cka[j][i] = kernel_cka[i][j]

    score_by_align_loss = np.zeros(num_clients)
    # score_by_kernel_cka = np.zeros(num_clients)
    # score_by_linear_cka = np.zeros(num_clients)

    for i in range(num_clients):
        score_by_align_loss[i] = np.sum(np.sort(align_loss_vals[i])[:])
        # score_by_linear_cka[i] = np.sum(np.sort(linear_cka[i])[:]) 
        # score_by_kernel_cka[i] = np.sum(np.sort(kernel_cka[i])[:]) 
    
    print(score_by_align_loss)
    selected_idx = np.argsort(score_by_align_loss)[:int(num_clients/2)]
...
#     selected_idx = np.arange(0, num_clients).tolist()
#     argsort_align_loss_score = np.flip(np.argsort(score_by_align_loss))
#     argsort_kernel_cka_score = np.argsort(score_by_kernel_cka)
    
#     print(argsort_align_loss_score)
#     print(argsort_kernel_cka_score)
#     for i in range(int(num_clients/2)-1):
#         if argsort_align_loss_score[i] == argsort_kernel_cka_score[i]:
#             selected_idx.remove(argsort_align_loss_score[i])

# #     for i in range(len(score_by_kernel_cka)):
        
# #         cka_mean = np.mean(score_by_kernel_cka)
# #         align_mean = np.mean(score_by_align_loss)

# #         if score_by_kernel_cka[i] < cka_mean:
# #             selected_idx.remove(i)
# #             # if score_by_align_loss[i] < align_mean:
